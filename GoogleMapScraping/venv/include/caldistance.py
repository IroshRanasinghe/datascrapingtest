...
while c <= len(CondoName):
    ...
    try:
        while CondoName[c] == CondoName[
            c - 1]:  # if the next row is the same condo, copy over information. Exit loop condo name mismatch.
            NearestStation.append([station, dist])
            print(str(c) + " The Nearest Train Station for " + CondoName[c] + " is " + station, str(dist) + " m.")
            c += 1
        print('============= Next Condo')
...
if __name__ == '__main__':
    distance_cal(7.623702, 79.916865,7.738455, 81.501330)