...
def main():
    ...
    if args.include_truncate:
        print
        "TRUNCATE TABLE {output_table};".format(output_table=args.output_table)

    print
    "CREATE TEMPORARY TABLE boxes ( id serial );"
    print
    "SELECT AddGeometryColumn('boxes', 'box', 4326, 'POLYGON', 2);"
    print
    "create index boxes__box on boxes USING GIST (box);"
    print
    "create unique index boxes__id on boxes (id);"

    for geom in box_coords(top=args.top, bottom=args.bottom, left=args.left, right=args.right,
    ...
    print
    "CREATE TEMPORARY TABLE ungrouped_output ( id serial, boxid integer );"
    print
    "SELECT AddGeometryColumn('ungrouped_output', 'geom', 4326, 'MULTIPOLYGON', 2);"
    ...
    if args.aggregate == 'collect':
        print
        "INSERT into {output_table} ({output_column}) SELECT ST_Multi(ST_CollectionExtract(ST_Collect(ungrouped_output.geom), 3)) as {output_column} FROM ungrouped_output GROUP BY boxid;".format(
            output_table=args.output_table, output_column=args.output_column)
        ...
        if args.include_truncate:
            ...
            "INSERT into {output_table} ({output_column}) SELECT ST_Multi(ST_CollectionExtract(ST_Collect(ungrouped_output.geom), 3)) as {output_column} FROM ungrouped_output GROUP BY boxid;".format(
                output_table=args.output_table, output_column=args.output_column)
            print
            "UPDATE {output_table} SET {output_column} = ST_Multi(ST_UnaryUnion({output_column}));".format(
                output_table=args.output_table, output_column=args.output_column)
        else:
            print
            "INSERT into {output_table} ({output_column}) SELECT ST_Multi(ST_CollectionExtract(ST_Union(ungrouped_output.geom), 3)) as {output_column} FROM ungrouped_output GROUP BY boxid;".format(
                output_table=args.output_table, output_column=args.output_column)
    else:
        raise NotImplementedError

    print
    "DROP TABLE ungrouped_output;"
    print
    "COMMIT;"
